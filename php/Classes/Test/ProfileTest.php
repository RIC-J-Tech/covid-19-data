...
//get count of Profile records in db before we run the test
	...
	//Returns an array
	$result = Profile::getProfileByUsername($this->getPDO(),$profile->getProfileUsername());

	//result may return multiple objects therefore we use the index 0
	$pdoProfile = $result[0];
	$this->assertEquals($this->VALID_PROFILE_USERNAME, $pdoProfile->getProfileUsername());
